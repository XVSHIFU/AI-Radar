...
const promptTextarea = ref<HTMLTextAreaElement | null>(null), copyFeedback = ref<{ ok: boolean; message: string } | null>(null);
...
const selectedSummary = computed(() => {
  if (!selectedCount.value) return tr("尚未选择文章", "No articles selected");
  const titles = selected.value.map(id => articles.value.find(item => item.article_version_id === id)?.title ?? id).join("、");
  return tr("已选 {count} 篇：{titles}", "{count} selected: {titles}", { count: selectedCount.value, titles });
});
...
async function exportPrompt() {
  ...
  try {
    ...
    if (!mounted) return;
    exportedPrompt.value = result.prompt; copyFeedback.value = null;
...
async function copyPrompt() {
  ...
  if (window.isSecureContext && navigator.clipboard?.writeText) {
    try { await navigator.clipboard.writeText(exportedPrompt.value); copied = true; }
    catch { /* Try the selection fallback. */ }
  }
  if (!copied && promptTextarea.value) {
    promptTextarea.value.focus({ preventScroll: true });
    promptTextarea.value.select();
    try { copied = document.execCommand("copy") === true; }
    catch { copied = false; }
  }
  copyFeedback.value = copied
    ? { ok: true, message: tr("任务已复制到剪贴板。", "Task copied to clipboard.") }
    : { ok: false, message: tr("浏览器未允许复制。任务文本已选中，可按 Ctrl+C 手动复制，或下载任务文件。", "Browser copy was unavailable. The task text is selected; press Ctrl+C or download the file.") };
}
...
<template>
  <section class="content-workbench admin-stack" :aria-label="tr('内容工作台', 'Content Workbench')">
    <header class="content-workbench__heading">
      ...
      <button class="admin-action" type="button" :disabled="!!busy || loading" @click="refresh">{{ tr('刷新', 'Refresh') }}</button>
    </header>
    ...
    <template v-else>
      ...
      <div v-if="batchActive" class="admin-notice content-workbench__running" role="status"><span>{{ tr('内容 Agent 批次正在运行', 'Content Agent batch is active') }}：{{ readableStatus(batch?.status) }}</span><button class="admin-action" type="button" :disabled="!!busy" @click="pauseBatch">{{ tr('暂停剩余任务', 'Pause remaining') }}</button></div>
      <section class="card content-workbench__queue">
        <div class="admin-section-title"><div><h3>{{ tr('选择文章', 'Select articles') }}</h3><p class="meta">{{ tr('每批最多 5 篇；仅处理本次勾选的冻结版本。已发布文章不能再提交；自动结果未知时仍可手动导出，不能自动重试。', 'Up to five per batch. Only selected frozen versions are processed. Published articles cannot be submitted again. Unknown Agent outcomes can still be exported manually, but cannot be retried automatically.') }}</p></div><div class="content-workbench__queue-controls"><span class="content-workbench__count">{{ selectedCount }}/5</span><button class="admin-action" type="button" :aria-expanded="queueExpanded" aria-controls="content-article-list" @click="queueExpanded = !queueExpanded">{{ queueExpanded ? tr('收起文章', 'Collapse articles') : tr('展开文章', 'Expand articles') }}</button></div></div>
        <p v-if="!queueExpanded" class="content-workbench__selection-summary" role="status">{{ selectedSummary }}</p>
        <div v-show="queueExpanded" id="content-article-list" class="content-workbench__queue-content"><p v-if="!articles.length" class="empty meta">{{ tr('暂无可处理的冻结文章。先在来源页采集文章，刷新后再选择。', 'No frozen articles are available. Ingest a source first, then refresh this list.') }}</p>
      ...
      <section v-if="mode === 'manual'" class="card content-workbench__flow">
        <div class="admin-section-title"><div><h3>{{ tr('导出任务', 'Export task') }}</h3><p class="meta">{{ tr('任务包含所选公开文章的原文与段落标记，不含 API 密钥或研究会话。', 'The task contains selected public articles and paragraph markers, with no API keys or research sessions.') }}</p></div><button class="admin-action" type="button" :disabled="!!busy || !selectedCount" @click="exportPrompt">{{ busy === 'export' ? tr('生成中…', 'Preparing…') : tr('生成摘要任务', 'Prepare summary task') }}</button></div>
        <div v-if="exportedPrompt" class="content-workbench__prompt"><div class="content-workbench__actions"><button class="admin-action" type="button" @click="copyPrompt">{{ copyFeedback?.ok ? tr('已复制任务', 'Task copied') : tr('复制任务', 'Copy task') }}</button><button class="admin-action" type="button" @click="downloadPrompt">{{ tr('下载任务文件', 'Download task file') }}</button></div><p v-if="copyFeedback" class="content-workbench__copy-feedback" :class="{ 'content-workbench__copy-feedback--error': !copyFeedback.ok }" :role="copyFeedback.ok ? 'status' : 'alert'" :aria-live="copyFeedback.ok ? 'polite' : 'assertive'">{{ copyFeedback.message }}</p><label>{{ tr('任务文本', 'Task text') }}<textarea ref="promptTextarea" class="control" :value="exportedPrompt" readonly rows="8" @focus="($event.target as HTMLTextAreaElement).select()" /></label></div>
        <div class="content-workbench__import"><h3>{{ tr('导入结果', 'Import results') }}</h3><p class="meta">{{ tr('粘贴网页聊天工具返回的 JSON；支持 JSON 代码块。每篇独立显示成功或错误，导入不自动发布。', 'Paste JSON from a web chat, including a JSON code block. Each article gets its own result. Importing never publishes automatically.') }}</p><label class="content-workbench__full">{{ tr('JSON 回答', 'JSON answer') }}<textarea v-model="importText" class="control" rows="8" spellcheck="false" :disabled="!!busy" :placeholder="tr('在此粘贴包含 task_id 的结果', 'Paste results containing task_id here')" /></label><button class="admin-action" type="button" :disabled="!!busy || !importText.trim()" @click="importResult">{{ busy === 'import' ? tr('校验中…', 'Validating…') : tr('校验并导入', 'Validate and import') }}</button>
      ...
      <section v-if="draft && editor" class="card content-workbench__draft" aria-labelledby="content-draft-title"><div class="admin-section-title"><div><h3 id="content-draft-title">{{ tr('草稿预览与审阅', 'Draft preview and review') }}</h3><p class="meta">{{ draft.article?.title }} · {{ draft.article?.source_url }}</p></div><button class="admin-action" type="button" :disabled="!!busy" @click="closeDraft()">{{ tr('关闭', 'Close') }}</button></div>
        ...
        <div class="content-workbench__actions content-workbench__footer"><button class="admin-action" type="button" :disabled="!!busy || !draftChanged" @click="saveDraft">{{ busy === 'save-draft' ? tr('保存中…', 'Saving…') : tr('保存草稿', 'Save draft') }}</button><button class="admin-action" v-if="!editor.relevant" type="button" :disabled="!!busy" @click="skipDraft">{{ busy === 'skip' ? tr('跳过中…', 'Skipping…') : tr('标为无关并跳过', 'Mark irrelevant and skip') }}</button><button type="button" class="content-workbench__publish" :disabled="!!busy || draftChanged || !editor.relevant || !!draft.validation_errors?.length || draft.status === 'published'" @click="publishDraft">{{ busy === 'publish' ? tr('发布中…', 'Publishing…') : tr('确认发布', 'Publish') }}</button><span v-if="draftChanged" class="meta">{{ tr('先保存修改，再发布。', 'Save changes before publishing.') }}</span></div>
      </section>
    </template>
  </section>
</template>
