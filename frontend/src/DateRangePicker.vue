...
const id=useId(),open=ref(false),trigger=ref<HTMLButtonElement>(),panel=ref<HTMLElement>(),startInput=ref<HTMLInputElement>(),draftFrom=ref(""),draftTo=ref(""),month=ref(""),choosingEnd=ref(false),rawError=ref(""),position=ref({left:"12px",top:"100px"});
...
const error=computed(()=>{switch(rawError.value){case "请输入有效日期。":return tr("请输入有效日期。","Enter a valid date.");case "请选择完整的开始和结束日期。":return tr("请选择完整的开始和结束日期。","Choose both a start and end date.");case "开始日期不能晚于结束日期。":return tr("日期范围无效：起始日期不能晚于截止日期。","Invalid date range: the start date must not be after the end date.");default:return rawError.value.startsWith("日期范围最多 ")?tr("日期范围最多 {days} 天，请缩小范围。","Date range can be at most {days} days.",{days:props.maxDays}):rawError.value}});
...
async function show(){draftFrom.value=props.from||shanghaiToday();draftTo.value=props.to||shanghaiToday();month.value=draftTo.value.slice(0,7);choosingEnd.value=false;rawError.value="";open.value=true;await nextTick();place();startInput.value?.focus()}
function close(returnFocus=true){open.value=false;if(returnFocus)trigger.value?.focus()}
function choosePreset(mode:DatePreset){const r=datePreset(mode);draftFrom.value=r.from;draftTo.value=r.to;month.value=r.to.slice(0,7);choosingEnd.value=false;rawError.value=""}
...
async function chooseDay(day:string){if(!choosingEnd.value){draftFrom.value=day;draftTo.value=day;choosingEnd.value=true}else{const start=draftFrom.value;draftFrom.value=day<start?day:start;draftTo.value=day<start?start:day;choosingEnd.value=false}rawError.value="";await nextTick();panel.value?.querySelector<HTMLButtonElement>('[data-day="'+day+'"]')?.focus()}
function apply(){const range={from:draftFrom.value,to:draftTo.value};rawError.value=validateRange(range,props.allowUnbounded,props.maxDays);if(error.value)return;emit("change",range);close()}
