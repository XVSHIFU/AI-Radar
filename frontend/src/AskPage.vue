...
function cancel() { answerGeneration++; controller.value?.abort(); running.value = false; status.value = "已取消，未自动重试。"; }
...
onBeforeUnmount(() => { answerGeneration++; overviewGeneration++; controller.value?.abort(); overviewController?.abort(); ruleController?.abort(); clearTimeout(timer); });
</script>
...
<template>
  <section class="ask-overview">
    ...
    <section class="ask-question"><h2>对这个范围提问</h2><label>问题<textarea v-model="question" class="control" rows="4" placeholder="输入需要核查的 AI 进展问题" /></label><div class="row"><button class="primary" :disabled="running || !question || invalid || tooWide" @click="submit">开始分析</button><button :disabled="planning || !question" @click="planFromQuestion">按问题筛选（规则解析）</button><button v-if="running" @click="cancel">取消</button></div><p v-if="ruleError" class="error" role="alert">{{ ruleError }}</p><section v-if="rulePlan" class="query-plan"><p>规则解析：分类 {{ rulePlan.filters.category || "沿用当前" }} · 日期 {{ rulePlan.filters.date_from || from }} 至 {{ rulePlan.filters.date_to || to }}</p><p v-if="rulePlan.requires_clarification || rulePlan.free_text || rulePlan.filters.entity_ids?.length" class="meta">部分条件无法完整映射到当前筛选，请改用筛选控件。</p><button @click="applyRulePlan">应用到总览</button></section><p class="meta" aria-live="polite">{{ status }}</p><div v-if="error" class="card error" role="alert"><strong>{{ error.code }}</strong>：{{ error.message }}</div><section v-if="plan" class="card query-plan" data-testid="query-plan"><h2>检索范围</h2><p>分类：{{ plan.filters.category || '全部' }} · 日期：{{ plan.filters.date_from || '不限' }} 至 {{ plan.filters.date_to || '不限' }}</p></section><article v-if="result || tokens" class="evidence-layer" data-testid="ask-answer"><p v-if="isDemo()" class="demo">模拟流，仅用于演示。</p><h2>回答</h2><p class="answer-body">{{ tokens || result?.answer }}</p><div v-for="source in sources" :key="source.index" class="evidence-item"><button :aria-expanded="expanded === source.index" @click="toggle(source.index)">[{{ source.index }}] {{ source.title }}</button><blockquote v-if="expanded === source.index" :id="`citation-${source.index}`" tabindex="-1">{{ source.quote_text || '无段落摘录' }}</blockquote><a v-if="validUrl(source.source_url)" :href="source.source_url" target="_blank" rel="noopener">打开来源</a></div><p v-if="metrics" class="meta">完整匹配 {{ metrics.scope_total }} · 覆盖：{{ metrics.coverage }}</p></article></section>
