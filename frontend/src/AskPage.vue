      ...
      <div v-else class="ask-charts">
        <section class="ask-chart" data-testid="insights-visual" :data-view="chartView">
          ...
          <template v-if="chartView === 'flow'"><p><button :disabled="motionReduced" :aria-pressed="flowPlaying" @click="toggleFlow">{{ flowPlaying ? "暂停流动" : "播放流动" }}</button><span v-if="motionReduced" class="meta">已遵循减少动态效果偏好。</span></p>
            <p class="chart-mobile-range">日期 {{ jointBuckets[0]?.label }} 至 {{ jointBuckets[jointBuckets.length - 1]?.label }}<span> · 左右滑动查看全部日期</span></p><div class="chart-scroll" aria-label="可横向滚动的分类流向图"><svg class="native-chart"  :viewBox="`0 0 720 ${flowHeight}`" :style="{ height: flowHeight + `px` }" role="img" aria-label="分类到日期的事件流向图">
            ...
            <p class="chart-mobile-range">日期 {{ jointBuckets[0]?.label }} 至 {{ jointBuckets[jointBuckets.length - 1]?.label }}<span> · 左右滑动查看全部日期</span></p><div class="chart-scroll" aria-label="可横向滚动的分类堆叠面积图"><svg class="native-chart" viewBox="0 0 720 190" role="img" aria-label="分类堆叠面积图">
          ...
          <template v-else>
            <p class="chart-mobile-range">左右滑动查看全部日期</p><div class="heatmap" role="grid" aria-label="日期和分类热力矩阵" :style="{ gridTemplateColumns: `minmax(90px, auto) repeat(${jointBuckets.length}, minmax(56px, 1fr))` }">
