...
<template>
  <section class="maintenance" :aria-busy="busy || loading">
    <div class="admin-section-title"><div><h2>{{ tr('数据核查', 'Data review') }}</h2><p class="meta">{{ tr('先查看保存的证据，再确认日期或归并。修改均保留审计记录。', 'Review saved evidence before correcting a date or merging events. Changes are audited.') }}</p></div><button class="admin-action" :disabled="busy || loading" @click="refresh">{{ loading ? tr('读取中…', 'Loading…') : tr('刷新', 'Refresh') }}</button></div>
    ...
    <details class="maintenance-section"><summary>{{ tr('人工归并重复事件', 'Merge duplicate events') }}</summary><p class="meta">{{ tr('从事件地址中复制编号。目标事件保留在列表中，两侧原文与证据保持可追溯。', 'Copy IDs from event URLs. The target remains visible and all sources stay traceable.') }}</p>
      <form class="maintenance-fields" @submit.prevent="previewMerge"><label>{{ tr('待归并事件编号', 'Source event ID') }}<input v-model="sourceId" class="control" :disabled="busy" @input="pair = []" /></label><label>{{ tr('保留的事件编号', 'Target event ID') }}<input v-model="targetId" class="control" :disabled="busy" @input="pair = []" /></label><button class="admin-action" :disabled="busy || !sourceId.trim() || !targetId.trim() || sourceId.trim() === targetId.trim()">{{ tr('对照事件', 'Compare events') }}</button></form>
      ...
      <div v-if="pair.length === 2"><label>{{ tr('归并依据', 'Reason for merging') }}<textarea v-model="reason" class="control" maxlength="2000" :disabled="busy" /></label><label class="maintenance-confirm"><input v-model="mergeConfirmed" type="checkbox" :disabled="busy" />{{ tr('已核对两侧证据，确认是同一事件。', 'I reviewed both sets of evidence and confirmed the same event.') }}</label><button class="admin-action" :disabled="busy || !mergeConfirmed || reason.trim().length < 3" @click="merge">{{ tr('确认归并', 'Confirm merge') }}</button></div>
      <ol v-if="merges.length" class="maintenance-audit"><li v-for="record in merges" :key="record.id"><strong>{{ record.source_title }} → {{ record.target_title }}</strong><p class="meta">{{ record.reason }}</p><p><time>{{ formatDate(record.created_at, { dateStyle: 'short', timeStyle: 'short' }) }}</time> · {{ record.reverted_at ? tr('已撤销', 'Reverted') : tr('已归并', 'Merged') }}</p><button class="admin-action" v-if="!record.reverted_at" :disabled="busy" @click="undoId = record.id">{{ tr('选择此记录以撤销', 'Select this record to undo') }}</button></li></ol>
      <form class="maintenance-fields" @submit.prevent="revert"><label>{{ tr('归并记录编号', 'Merge record ID') }}<input v-model="undoId" class="control" :disabled="busy" /></label><button class="admin-action" :disabled="busy || !undoId.trim()">{{ tr('撤销此归并', 'Undo this merge') }}</button></form>
    </details>
