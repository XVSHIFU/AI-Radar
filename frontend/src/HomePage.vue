  ...
  <section>
    ...
    <div class="two">
      <aside class="card">
        ...
        <div id="advanced" v-show="!compact || advanced">
          <fieldset>
            <legend>分类</legend>
            <label v-for="c in categories" :key="c.v" class="row"
      ...
      <div>
        ...
        <p v-else aria-live="polite" class="meta">
          {{ loading ? "正在更新匹配结果…" : `精确匹配 ${total} 条事件` }}
        </p>
        <div v-if="error" class="card error" role="alert">
          {{ error.message }}<button @click="load()">重试</button>
        </div>
        <div v-else-if="!loading && !items.length" class="card empty">
          这个范围内没有事件。
        </div>
        <article v-for="x in items" :key="x.id" class="card event">
          <span class="pill">{{
            categories.find((c) => c.v === x.category)?.l
          }}</span
          ><span class="meta"> {{ x.event_date || "日期未知" }}</span>
          <h2>
            <RouterLink
              :to="{
                path: `/events/${x.id}`,
