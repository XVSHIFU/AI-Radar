...
try {
    ...
    $report=[pscustomobject]@{started_at=$startedAt.ToString('o');finished_at=[DateTimeOffset]::UtcNow.ToString('o');layer='local_static_unit_offline_migration_build';postgres_integration_executed=$false;live_model_executed=$false;passed=@($checks|Where-Object passed).Count;expected_steps=10;checks=$checks}
