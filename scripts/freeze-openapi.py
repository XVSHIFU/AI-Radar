...
def main() -> int:
    ...
    parser.add_argument(
        "--write", action="store_true", help="Update after contract review"
    )
    ...
    if args.write:
        ...
        return 0
    if not target.exists() or json.loads(
        target.read_text(encoding="utf-8")
    ) != json.loads(schema):
        print(
            "OpenAPI differs from reviewed contracts/openapi/v1.json. Review before --write."
        )
        return 1
