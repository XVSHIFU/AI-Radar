...
$observations = foreach ($sessionFile in $recentFiles) {
    ...
    foreach ($line in $records) {
        ...
        foreach ($window in @($entry.payload.rate_limits.primary, $entry.payload.rate_limits.secondary)) {
            if ($null -ne $window) {
                [pscustomobject]@{observed_at=$entry.timestamp; window_minutes=$window.window_minutes; remaining_percent=(100 - $window.used_percent); stop_rule_enabled=$false}
            }
        }
    }
}
...
if ($latest.Count -eq 0) { Write-Output '{"status":"unknown","stop_rule_enabled":false}'; exit 0 }
...
if ($stale) { Write-Output '{"status":"stale","stop_rule_enabled":false}' }
