...
async def canonical_counts(session: AsyncSession, canonical_id: UUID) -> tuple[int, int]:
    ...
    candidate_sources = (
        select(ArticleCandidateRow.source_id)
        .select_from(EventArticleRow)
        .join(ArticleVersionRow, ArticleVersionRow.article_id == EventArticleRow.article_id)
        .join(ArticleCandidateRow, ArticleCandidateRow.article_version_id == ArticleVersionRow.id)
        .where(EventArticleRow.event_id.in_(member_ids))
    )
    all_sources = direct_sources.union(candidate_sources).subquery()
    source_count = int((await session.scalar(select(func.count()).select_from(all_sources))) or 0)
    evidence_count = int(
        (
        ...
        *,
        ...
        async with self.sessions() as session, session.begin():
            ...
            source_has_members = await session.scalar(
                select(EventRow.id).where(EventRow.merged_into_event_id == source_id).limit(1)
            )
            ...
            await session.flush()
            target.source_count, target.evidence_count = await canonical_counts(session, target_id)
    ...
    async def unmerge(self, log_id: UUID, *, operator: str) -> None:
        ...
        async with self.sessions() as session, session.begin():
            ...
            log = await session.scalar(
                select(EventMergeLogRow).where(EventMergeLogRow.id == log_id).with_for_update()
            )
            ...
            await session.flush()
            target.source_count, target.evidence_count = await canonical_counts(session, target.id)
            target.content_version += 1
            target.updated_at = datetime.now(UTC)
