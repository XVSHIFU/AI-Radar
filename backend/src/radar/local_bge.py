...
class LocalBgeM3Provider:
    ...
    def __init__(self, model_dir: Path, revision: str, *, threads: int = 4) -> None:
        ...
        self.profile = EmbeddingProfile(
            ...
            normalize=True,
            input_template_version="plain-cls512-v2",
        )
    ...
    def _embed(self, text: str) -> list[float]:
        ...
        # BGE-M3 dense embeddings use CLS, as specified by the upstream pooling config.
        pooled = hidden[:, 0, :].copy()
    ...
    def _serialized_embed(self, text: str) -> list[float]:
        # Bound CPU inference and prevent duplicate lazy model loads on this small VM.
        with self._inference_lock:
            return self._embed(text)

    async def embed_query(self, text: str) -> list[float]:
        return await asyncio.to_thread(self._serialized_embed, text)
