...
def upgrade() -> None:
    op.add_column(
        "events",
        sa.Column(
            "date_basis", sa.String(32), nullable=False, server_default="report_date_unverified"
        ),
    )
    ...
    op.create_foreign_key(
        "fk_events_date_evidence", "events", "evidence", ["date_evidence_id"], ["id"]
    )
    op.create_foreign_key(
        "fk_events_merged_into", "events", "events", ["merged_into_event_id"], ["id"]
    )
    op.create_check_constraint(
        "ck_events_date_semantics",
        "events",
        "(date_precision = 'unknown' AND event_date IS NULL) OR "
        "(date_precision IN ('day', 'month') AND event_date IS NOT NULL)",
    )
    ...
    op.add_column(
        "evidence",
        sa.Column("support_type", sa.String(24), nullable=False, server_default="direct"),
    )
    op.execute(
        "UPDATE evidence SET quote_hash = encode(sha256(convert_to(quote_text, 'UTF8')), 'hex') "
...
def downgrade() -> None:
    active_merge = (
        op.get_bind()
        .execute(sa.text("SELECT 1 FROM event_merge_log WHERE reverted_at IS NULL LIMIT 1"))
        .scalar()
    )
