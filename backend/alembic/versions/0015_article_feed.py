...
def upgrade() -> None:
    ...
    op.add_column(
        "articles", sa.Column("status", sa.String(16), nullable=False, server_default="legacy")
    )
    ...
    for table in ("articles", "article_versions", "event_articles", "sources"):
        op.execute(
            f"CREATE TRIGGER trg_{table}_feed_revision "
            f"AFTER INSERT OR UPDATE OR DELETE ON {table} "
            "FOR EACH STATEMENT EXECUTE FUNCTION bump_retrieval_data_revision()"
        )
...
def downgrade() -> None:
    ...
    for name in (
        "duplicate_of_id", "content_hash", "current_version_id", "category",
        "status", "ingested_at", "published_at", "excerpt", "title",
    ):
