...
日期：2026-09-16。**单任务 gVisor 隔离、独立崩溃回收，以及独立 HTTP 网关→授权数据集→产物下载链路均已通过真机验收；pi 工具、来源与 SSE 产物已接入候选代码；生产健康发布、完整真机公开链路与上线仍未完成，Python 工具保持关闭。**
...
## pi 工具、分析依据与 SSE 产物接入（候选，2026-09-16）

- pi 与后端双重执行每轮最多一次 Python，计入四次业务工具调用；三次模型调用、固定输入/输出预算保持不变。默认不注册 Python。`policy.json` 仍为 enabled=false；验证器现在允许显式 true，但逐字段核对全部固定隔离参数和类型，拒绝 host 网络、提高内存/PID/guest_tasks 或删除安全字段。
- 后端只接受实际模型返回且参数完全相同的工具调用；从当前 `ResearchTools` 注册表提供数据，第三轮无可用工具。开启分析技能时只加载包内固定 `analyze-dataset` 文档，不增加插件发现或安装能力。
- 开启 Python 的公开问题在配额预留之前检查 pi 的同一策略摘要与 Python 状态，再检查私有控制器的固定镜像 ID、gVisor 二进制摘要和 watchdog 状态。控制器 URL 与凭证独立，检查有三秒总时限；失败返回 503 且不扣问题额度。**生产控制器尚未提供此健康发布入口，因此仅改策略文件为 true 也不会开放执行。**
- 分析结果注册单独的 analysis 引用，保存实际代码、受限 stdout、原数据集及来源索引；文件同时附 run、dataset、as_of、timezone 和十五分钟过期时间。输出控制字符经 JSON 转义后的真实体积也受限，不让工具消息超出模型返回预算。
- 公开 SSE 的 artifacts 事件只跟随已验证的正式 sources，并且只发出被最终回答引用的分析产物；普通 JSON 端点返回相同文件信息。引用伪造或研究失败不会发布正式文件事件。
- 前端校验事件顺序、run、引用编号、同源固定下载路径、UUID、文件名/MIME/大小与到期时间；文件只在 completed 后展示，草稿和错误结果不展示。PNG 预览、JSON/CSV 下载、过期和失败提示继承 A 布局 + E 时间线与主题。下载再检查实际响应 MIME 和字节上限，不跟随重定向。
- “依据”按钮打开对应分析详情，支持查看输入数据、分析输出与代码。手机宽度下长文件名换行，按钮保留可达尺寸；所有文字有中英文。预览资源与请求在组件卸载时释放，过期图片不继续显示。

### 验收范围

- 新增三轮模型协议测试：聚合→Python→回答，实际后端 capability/预算/模型消息重建及私有 ASGI HTTP 网关参与，模型与执行器为明确夹具；不能把这项称为真实模型/gVisor/生产数据库全链路。
- pi 真实 SDK 测试 15 项通过，含工具默认关闭、单次执行和完整隔离策略检查；前端 50 项通过，含跨 run 链接、伪造引用、文件到期、MIME 与字节超限；前端正式构建通过，八个后端模块严格 mypy 通过。
- 浏览器桌面 1440 与手机 390 合成数据检查完成；首次检查发现文件依据未展开外层 details，已修正并确认焦点落到对应引用，三行原数据可读。图表模拟响应仅存在于隔离浏览器，不改演示/生产数据。Impeccable 检测无新增告警。截图在本地 `.run/artifact-*-confirm.png`；早期加载失败是测试页重载先于模拟响应安装，下载失败提示也因此得到实际检查。

尚待：提供生产控制器的启动清理与可信健康发布；将真实 pi、PostgreSQL、gVisor、公开 SSE/下载串成一次完整隔离验收；P4 原有四十题质量与记忆规范，P5 全栈 Compose、独立备份恢复和实际上线。没有新增收费模型调用，也未对旧助手运行服务启用这些候选能力。
`n最终本地受影响回归：启用真实 pi 子进程回调测试后共 191 项通过，无跳过；所有模型上游为夹具。
