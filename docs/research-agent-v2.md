...
最新用户调整（2026-09-16）：公开助手改为同一 IP 滚动 48 小时最多 **5 次提问**，追问计入；不显示常驻剩余额度，仅用完时提示。以下旧版 20 次记载属于历史方案，以本条及当前 policy 为准。验证以个人开源项目可用为目标，优先零费用夹具，只运行受影响检查，不重复收费试验；不再把大规模付费质量评测当作默认开发步骤。
...
- 2026-09-16 后续候选进展：P2 公开控制器/SSE/只读工具/持久化模型账本已完成集成测试；P1 保留期新增 0013 并在隔离测试库验证。线上仍为原助手，真实供应商验证待确认。P3 协议、镜像构建文件和固定权限控制器已开始实现，状态与缺口见 [P3 开发记录](research-agent-p3-progress.md)。

- P3 真机约束补充：保留宿主 cgroup 的 32 PIDs 上限，另设 guest 的 `RLIMIT_NPROC=2:2`，含主进程在内最多两个进程/线程。此约束只收紧原上限，初版面向单 CPU 的 numpy/pandas/matplotlib 分析，不承诺任意多进程算法可用；必须复验正常分析后才接入公开工具。
